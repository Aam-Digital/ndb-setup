...
2. Edit the `docker-compose.yml`
   ...
   2. in `backend` set `DATABASE_PASSWORD` to a secure password. This is used for the replication process.
...
7. Visit `<VIRTUAL_HOST>/db/db/_utils/` and add a `Permission` entity to the database and define the user roles
...
2. Edit the `docker-compose.yml`
   1. in `app` set `VIRTUAL_HOST` and `LETSENCRYPT_HOST` to the desired URL and `LETSENCRYPT_EMAIL` to the certified email. Set `COUCHDB_URL` to `http://couchdb:5984`
